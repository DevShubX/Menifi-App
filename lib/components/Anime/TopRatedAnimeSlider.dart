...
import 'package:menifi/pages/AnimeScreens/TopRatedAnimeScreen.dart';

import '../../pages/AnimeScreens/AnimeSearchScreen.dart';
import '../Skeletons/ImagesSliderSkeleton.dart';

class TopRatedAnimeSlider extends StatefulWidget {
  const TopRatedAnimeSlider({super.key});

  @override
  State<TopRatedAnimeSlider> createState() => _TopRatedAnimeSliderState();
}

class _TopRatedAnimeSliderState extends State<TopRatedAnimeSlider> {
  int pageNumber = 1;
  late Map data;
  List? Anime = [];
  bool isLoading = true;
  ...
  Future getAnime() async {
    http.Response response = await http.get(Uri.parse(
        'https://redux-api-wine.vercel.app/api/top100?page=${pageNumber}&count=${count}'));
              ...
              GestureDetector(
                onTap: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => const TopRatedAnimeScreen()));
                },
                child: const Text(
                  "See All",
                  style: TextStyle(

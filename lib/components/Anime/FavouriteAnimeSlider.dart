...
import 'package:menifi/pages/AnimeScreens/FavouriteAnimeScreen.dart';

import '../../pages/AnimeScreens/AnimeSearchScreen.dart';
import '../Skeletons/ImagesSliderSkeleton.dart';

class FavouriteAnimeSlider extends StatefulWidget {
  const FavouriteAnimeSlider({super.key});

  @override
  State<FavouriteAnimeSlider> createState() => _FavouriteAnimeSliderState();
}

class _FavouriteAnimeSliderState extends State<FavouriteAnimeSlider> {
  int pageNumber = 1;
  late Map data;
  List? Anime = [];
  bool isLoading = true;
  ...
  Future getAnime() async {
    http.Response response = await http.get(Uri.parse(
        'https://redux-api-wine.vercel.app/api/favourite?page=${pageNumber}'));
              ...
              GestureDetector(
                onTap: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => const FavouriteAnimeScreen()));
                },
                child: const Text(
                  "See All",
                  style: TextStyle(

...
import 'package:menifi/pages/TVShowScreens/TrendingTvShowsScreen.dart';
...
class TrendingTvShowsSlider extends StatefulWidget {
  const TrendingTvShowsSlider({super.key});

  @override
  State<TrendingTvShowsSlider> createState() => _TrendingTvShowsSliderState();
}

class _TrendingTvShowsSliderState extends State<TrendingTvShowsSlider> {
  int pageNumber = 1;
  late Map data;
  List? TVShows = [];
  bool isLoading = true;
  ...
  Future getTVShows() async {
    http.Response response = await http.get(Uri.parse(
        'https://api.themoviedb.org/3/trending/tv/day?api_key=$key&language=en-US&page=$pageNumber'));
                ...
                GestureDetector(
                  onTap: () {
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => TrendingTvShowsScreen()));
                  },
                  child: const Text(
                    "See All",
                    style: TextStyle(

...
import 'package:menifi/pages/MangaScreens/TrendingMangaScreen.dart';
import 'dart:convert';
import 'dart:async';

import '../Skeletons/ImagesSliderSkeleton.dart';

class TrendingMangaSlider extends StatefulWidget {
  const TrendingMangaSlider({super.key});

  @override
  State<TrendingMangaSlider> createState() => _TrendingMangaSliderState();
}

class _TrendingMangaSliderState extends State<TrendingMangaSlider> {
  int pageNumber = 1;
  late Map data;
  List? Manga = [];
  bool isLoading = true;
              ...
              GestureDetector(
                onTap: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => TrendingMangaScreen()));
                },
                child: const Text(
                  "See All",
                  style: TextStyle(
                          ...
                          child: Column(
                            children: [
                              ...
                              Container(
                                ...
                                child: Text(
                                  "${Manga![index]['title']['romaji'] ?? Manga![index]['title']['english'] ?? Manga![index]['title']['userPreferred']}",
                                  style: TextStyle(
                                    fontFamily: 'Gilroy-Medium',
                                    fontSize: 13,
                                    color: Manga![index]['coverImage']
                                                ['color'] !=
                                            null
                                        ? Color(int.parse(Manga![index]
                                                ['coverImage']['color']
                                            .toString()
                                            .replaceAll("#", "0XFF")))
                                        : Colors.white,
